...
<head>
  ...
  <script>
    function markAsRead(query_id) {
        if (confirm("Do you really want to mark this as Read?")) {
            window.location.href = "mark_read.php?id=" + query_id;
        }
    }
  </script>
...
<div id="content">
  <div class="content-wrapper">
    <div class="container-fluid">
          ...
          <tbody>
            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
              <tr>
                ...
                <td>
                  <a href="#" onclick="markAsRead(<?php echo $row['query_id']; ?>)">
                    <b>Pending</b>
                  </a>
                </td>
      ...
      <?php } else { ?>
        <div class="alert alert-info"><b>No pending queries found.</b></div>
      <?php } ?>
      ...
      <div class="table-responsive" style="text-align:center;">
        ...
        if (mysqli_num_rows($result1) > 0) {
          ...
          if ($page > 1) {
            echo '<li><a href="pendingquer.php?page='.($page - 1).'">Prev</a></li>';
          }
          for ($i = 1; $i <= $total_page; $i++) {
            $active = ($i == $page) ? "active" : "";
            echo '<li class="'.$active.'"><a href="pendingquer.php?page='.$i.'">'.$i.'</a></li>';
          }
          if ($total_page > $page) {
            echo '<li><a href="pendingquer.php?page='.($page + 1).'">Next</a></li>';
          }
          echo '</ul>';
        }
        ?>
